...
class PSDMass(torch.nn.Module):
    '''A positive semi-definite matrix of the form LL^T + epsilon where L is a neural network'''
    def __init__(self, input_dim, hidden_dim, diag_dim, nonlinearity='tanh', init_gain = 1):
        super(PSDMass, self).__init__()
...
class PSDInertial(torch.nn.Module):
    '''A positive semi-definite matrix of the form LL^T + epsilon where L is a neural network'''

    def __init__(self, input_dim, hidden_dim, diag_dim, nonlinearity='tanh', init_gain=1):
        super(PSDInertial, self).__init__()
        self.diag_dim = diag_dim
        if diag_dim == 1:
            self.linear1 = torch.nn.Linear(input_dim, hidden_dim)
            self.linear2 = torch.nn.Linear(hidden_dim, hidden_dim)
...
class MatrixNet(torch.nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, nonlinearity='tanh', bias_bool=True, shape=(2,2), init_gain =1):
